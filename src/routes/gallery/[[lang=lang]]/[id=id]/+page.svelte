<script>
    ...
    $: storymapContent = allContent.tiles.filter(
        (f) => slugify(f.en.country) === storymapId
    )[0];
    $: storymapurl = storymapContent[lang].storymap;
    $: imgUrl = storymapContent.imagexy
        ? storymapContent.imagexy + "/max"
        : "full/,300";
</script>
...
<main>
    <div
        class="left"
        style="background-image: linear-gradient( rgba(var(--midnight), 0.7), rgba(var(--midnight), 0.7) ), url('{base}/gallery/{storymapId}.jpg');"
    >
        <div class="bg" />
        <div class="logo">
            <img
                src="{base}/NLogo_granite.png"
                height="50"
                width="50"
                alt={content.logoalt}
            />
            <a class="home-link" href="{base}/gallery/{$page.params.lang || ''}">
                {content.title}
            </a>
        </div>
        ...
        <div class="btn-box">
            <a
                class="home-btn"
                href="{base}/gallery/{$page.params.lang || ''}"
                style="background: #{storymapContent.color};"
                >{lang === "en"
            ...
            <div class="mini-btn-box">
                <a
                    class="lang-btn"
                    href="{base}/gallery/es/{$page.params.id}"
                    style="background: #{storymapContent.color};"
                    >{content.es}</a
                >
                <a
                    class="lang-btn"
                    href="{base}/gallery/{$page.params.id}"
                    style="background: #{storymapContent.color};"
                    >{content.en}</a
                >
            </div>
    ...
    <div class="right">
        <iframe src={storymapurl} frameborder="0" title="storymap i-frame" />
        <!-- {storymapurl} -->
    </div>
</main>
