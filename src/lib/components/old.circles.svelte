<script>
    ...
    $: left =
        $width && $width > 0
            ? Math.floor($width * (circe.x / 1450)) - 25
            : circe.x - 25;
    $: bottom =
        Math.min($width / 1450, ($height - (75 + $height * 0.35)) / 865) *
        (865 - y);
    // 0.65 * $height - 75
    // $: console.log(bottom)
...
<a
    href="{base}/{slugify(circe.en.country)}"
    id={circe.color}
    class="circle-wrap {cardHover === idx ? 'card-hover' : ''}"
    style="left: {left}px; bottom: {bottom - 25}px;"
>
    <svg width="50" height="50">
        ...
        <text
            ...
            y="37"
            text-anchor="middle"
            stroke="rgb(var(--midnight))"
            stroke-width="1px"
            font-size="33px">{idx + 1}</text
        >
    </svg>
</a>
